...
export default new Vuex.Store({
  ...
  getters: {
    ...
    daily(state) {
      return state.weather.daily.data.map(el => {
        let str = el.icon;
        if (el.cloudCover > 0.6 && str.startsWith("partly")) str = str.replace("partly", "mostly");
  ...
  mutations: {
    ...
    addLocation(state, obj) {
      ...
      state.recentLocations = state.recentLocations.slice(0, MAX_RECENT_LOCATIONS);
    },
  ...
  actions: {
    ...
    async getIp({ dispatch, commit }) {
      ...
      // let ipapi = `https://cors-anywhere.herokuapp.com/http://ip-api.com/json/${ipv4}?fields=57823`;
    ...
    getReverseTomtom({ commit }, payload) {
      // #region dynamic dev ----------- // 
...
function formatTomtom(data) {
  ...
  obj.regionName = data.address.countrySubdivisionName ?
    data.address.countrySubdivisionName : data.address.countrySubdivision;

  obj.country = data.address.country;
  obj.countryCode = data.address.countryCode;
  obj.localName = data.address.localName ? data.address.localName : data.address.freeformAddress ? data.address.freeformAddress : data.address.countrySubdivisionName;
